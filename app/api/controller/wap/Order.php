...
class Order extends Base {
    ...
    public function service(){
        ...
        $result = Db::name("order_refundment")
            ->alias("r")
            ->field("o.*,r.order_goods_id,r.amount,r.pay_status as r_pay_status,r.create_time as r_create_time")
            ->join("order o","r.order_id=o.id","LEFT")
            ->order('r.id desc')
        ...
        foreach($result as $key=>$value){
            ...
            $order_goods_id = explode(',',$value['order_goods_id']);
            $goods = Db::name("order_goods")->where("order_id",'in',$order_goods_id)->select()->toArray();
            foreach($goods as $k=>$v){
                $goods_array = json_decode($v["goods_array"],true);
                $data[$key]['item'][$k] = [
                    "title"=>$goods_array["title"],
